...
class Test_UserView(APITestCase):
    ...
    def test_delete_store(self):
        url = "http://127.0.0.1:8000/api/users/"+ str(self.userExists.pk)+ "/delete_store/"
        store_id = self.store2.pk
        # print(f"test: {self.userExists.stores.all()}")
        ...
        self.assertFalse(Association.objects.filter(user=self.userExists, store__pk=store_id).exists())
